...
def explore_anomalies(anomalies, anomaly_descriptions):
    for i, (index, anomaly_row) in enumerate(anomalies.iterrows(), 1):
        print(f"\nAnomaly {i}:")
        print(f"Timestamp: {anomaly_row['timestamp']}")
        print(f"Value: {anomaly_row['value']}")
        print(f"Description: {anomaly_descriptions[i-1]}")

        # Allow user to interactively ask questions
        user_question = input("\nEnter your question about this anomaly (or type 'exit' to end): ")
        if user_question.lower() == 'exit':
            break

        # Use ChatGPT to generate responses to user questions
        prompt = f"Explain the anomaly in the time series data at timestamp {anomaly_row['timestamp']} with a value of {anomaly_row['value']}. The context is {query}. User's question: {user_question}"
        response = openai.Completion.create(
            engine="text-davinci-002",
            prompt=prompt,
...
explore_anomalies(anomalies, anomaly_descriptions)
...
try:
    anomaly_info, anomalies, anomaly_descriptions = detect_anomalies_and_describe(query, file_path)
    print(anomaly_info)
    # Print or use the generated descriptions
    print("Anomaly Descriptions:")
    for i, description in enumerate(anomaly_descriptions, 1):
        print(f"Anomaly {i}: {description}")
